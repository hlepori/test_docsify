...
3.7.1.4	An important principle behind the introduction of a globally unique identifier (PANS-ATM 17.4.1) is that it should identify only one flight and each flight should be identified by a single identifier. If a flight is cancelled, for whatever reason, and a new flight plan is submitted, the new flight shallbe assigned a new GUFI even if the new flight plan relates to what the operator will consider to be the same flight. Alternatively, in place of the cancel and re-file option, the new flight plan can be provided with the same GUFI but with an incremented flight plan version and as a consequence it will be treated as a complete replacement of the existing data. See 3.7.4.3 and 6.4.3.7.
